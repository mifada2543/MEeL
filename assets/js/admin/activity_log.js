...
(function () {
  ...
  window.previewExport = function (format) {
    ...
    var formatIcons = { csv: 'file-down', json: 'file-code', xls: 'file-spreadsheet' };
      ...
      .then(function (data) {
        ...
        Swal.fire({
          ...
          html: '<div style="text-align:left">'
            ...
            + '<span><svg style="width:12px;height:12px;display:inline;margin-right:4px;vertical-align:middle" data-lucide="list" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>Menampilkan ' + data.preview_count + ' dari ' + data.total + ' baris</span>'
            + '<span style="font-family:ui-monospace,monospace;color:' + accentColors[format] + '">.' + format + '</span>'
            + '</div>'
            + '<pre style="max-height:420px;overflow-y:auto;background:#090c12;border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:14px;font-size:11px;line-height:1.7;font-family:ui-monospace,monospace;color:#d1d5db;white-space:pre-wrap;word-break:break-all;margin:0 0 12px 0">' + escaped + '</pre>'
            + '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px">'
            + '<span style="font-size:9px;color:#4b5563">Total ' + data.total + ' baris</span>'
            + '<button id="swal-download-btn" style="background:#1e293b;border:1px solid rgba(255,255,255,.08);color:#fff;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.1em;padding:8px 18px;border-radius:10px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:all .15s" data-href="' + downloadUrl + '">'
            + '<svg style="width:14px;height:14px" data-lucide="' + formatIcons[format] + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v14"/><path d="m19 9-7 7-7-7"/><path d="M5 21h14"/></svg> Download ' + formatLabels[format]
            + '</button>'
            + '</div>'
            + '</div>',
          background: '#0e1118',
