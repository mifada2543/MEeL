...
(function () {
  ...
  window.showSection = function (id, btn, isMobile) {
    ...
    if (btn) {
      if (isMobile) {
        ...
        var dtBtn = document.querySelector('.nav-btn-desktop[onclick*="' + id + '"]');
        if (dtBtn) {
        ...
        var mbBtn = document.querySelector('.nav-btn-mobile[onclick*="' + id + '"]');
        if (mbBtn) {
  ...
  // Initial load: activate Video section
