 ...
 * Self-executing closure to keep all internal state private.
 */
(function () {
  'use strict';
  ...
  var pendingProgress = null; // untuk rAF throttle
  var rafId = null;
  var currentXhr = null; // referensi XHR aktif untuk abort
  ...
  // ── Raf commit: tulis semua DOM update sekali per frame ──
  ...
  function showCard(filename) {
    ...
    // Paksa style recalculation biar animasi masuk jalan
  ...
  if (dropzone) {
    ...
    dropzone.addEventListener('drop', function (e) {
      ...
      // Build FormData manual (tidak bisa set input.files)
  ...
  form.addEventListener('submit', function (e) {
    ...
    // Capture FormData SEBELUM disable fileInput
    var formData = new FormData(form);

    startUpload(formData, file.name, file.size);
  });

  // ── Refresh Drive (tombol refresh) ──
  window.refreshDrive = function () {
    var btn = document.getElementById('refreshBtn');
    var icon = btn ? btn.querySelector('i') : null;
    if (icon) icon.classList.add('animate-spin');

    // Step 1: invalidate cache & ambil storage data fresh dari server
    fetch('../controllers/api/ajax_refresh.php?_=' + Date.now())
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (data.status === 'success' && typeof data.storage_usage !== 'undefined') {
          updateStorageBar(data.storage_usage, data.storage_percentage);
        }
      })
      .catch(function () {
        // Silent — grid refresh tetap jalan
      })
      .then(function () {
        // Step 2: refresh file grids dari page HTML
        return fetch(window.location.href + (window.location.href.indexOf('?') === -1 ? '?_=' : '&_=') + Date.now());
      })
      ...
      .then(function (html) {
        ...
        // Update heading file count jika section aktif
